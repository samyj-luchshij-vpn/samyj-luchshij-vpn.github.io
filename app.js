(() => {
  ...
  // ---------- Preloader ----------
  // Фикс: прелоадер скрывается и на load, и с запасным таймером (если где-то дальше будет ошибка)
  ...
  // запасной вариант — чтобы сайт не зависал на "готовим страницу"
...
// ---------- Mobile drawer ----------
  ...
  if (grid) {
    ...
    const isPinned = (card) => getStr(card, 'data-id') === 'vpntype';

    const sortCards = (arr, mode) => {
      return [...arr].sort((a, b) => {
        // VPNTYPE pinned всегда, кроме "хуже отзывы → лучше"
